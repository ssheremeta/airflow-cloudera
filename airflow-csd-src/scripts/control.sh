...
case ${SERVICE} in
  (webserver_initdb)
    ...
    exec su airflow -c "$AIRFLOW_CONDA_HOME/bin/airflow initdb" >> ${CONF_DIR}/logs/stdout.log 2>> ${CONF_DIR}/logs/stderr.log
    ;;
  (webserver_start)
    log "Starting the Airflow webserver"
    substitute_common_tokens
    substitute_webserver_tokens
    exec su airflow -c "$AIRFLOW_CONDA_HOME/bin/airflow webserver" >> ${CONF_DIR}/logs/stdout.log 2>> ${CONF_DIR}/logs/stderr.log
    ;;
  (celery_start)
    log "Starting the Airflow celery"
    substitute_common_tokens
    substitute_celery_tokens
    exec su airflow -c "$AIRFLOW_CONDA_HOME/bin/airflow flower" >> ${CONF_DIR}/logs/stdout.log 2>> ${CONF_DIR}/logs/stderr.log
    ;;
  (worker_start)
    log "Starting the Airflow worker"
    substitute_common_tokens
    exec su airflow -c "$AIRFLOW_CONDA_HOME/bin/airflow worker -q ${WORKER__QUEUE}" >> ${CONF_DIR}/logs/stdout.log 2>> ${CONF_DIR}/logs/stderr.log
    ;;
  (scheduler_start)
    log "Starting the Airflow scheduler"
    substitute_common_tokens
    substitute_scheduler_tokens
    exec su airflow -c "$AIRFLOW_CONDA_HOME/bin/airflow scheduler" >> ${CONF_DIR}/logs/stdout.log 2>> ${CONF_DIR}/logs/stderr.log
    ;;
  (worker_pip_install)
